 ...
 *
 * Defines the contract for algorithms that can decompress data in chunks
 * using Kotlin Flow. This approach is memory-efficient and suitable for
 * large compressed files or continuous data streams that cannot fit entirely in memory.
 *
 * The decoder processes compressed chunks incrementally, emitting decompressed
 * data as it becomes available, which allows for constant memory usage
 * regardless of compressed file size.
 *
...
interface StreamDecoder<Meta : MetaData> {
     ...
     *
     * Processes compressed data incrementally using the provided metadata
     * and emits decompressed chunks as they are produced.
     *
     * The implementation should handle chunk boundaries appropriately,
     * maintaining any necessary state between chunks to ensure correct
     * decompression.
     *
     ...
     * ```
     */
    suspend fun decode(compressed: CompressedStreamData<Meta>): Flow<ByteArray>
}
