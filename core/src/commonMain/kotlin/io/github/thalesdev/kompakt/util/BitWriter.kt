 ...
 *
 * Accumulates bits in a 64-bit buffer and flushes to byte array when necessary.
 * Optimized for minimal branching and efficient CPU pipeline utilization.
 *
 * @property bufferSize Size of the internal byte buffer (default 64KB)
 */
class BitWriter(private val bufferSize: Int = 65536) {
    @PublishedApi
    internal var bitBuffer = 0L
    @PublishedApi
    internal var bitCount = 0
    @PublishedApi
    internal val buffer = ByteArray(bufferSize)
    @PublishedApi
    internal var bufferIndex = 0

    /**
     * Writes bits to the buffer with minimal overhead.
     *
     * Accumulates bits in a 64-bit buffer and flushes to byte array when
     * the buffer contains 56 or more bits, ensuring space for subsequent writes.
     *
     ...
     *
     * Flushes remaining bits, adding padding if necessary to complete the last byte.
     * Resets the writer state after finishing.
     *
