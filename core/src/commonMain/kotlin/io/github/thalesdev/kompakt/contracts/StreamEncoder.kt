 ...
 *
 * Defines the contract for algorithms that can compress data in chunks using Kotlin Flow. This
 * approach is memory-efficient and suitable for large files or continuous data streams that cannot
 * fit entirely in memory.
 *
 * The encoder processes data incrementally, emitting compressed chunks as they become available,
 * which allows for constant memory usage regardless of input size.
 *
...
interface StreamEncoder<Meta : MetaData> {
   ...
   *
   * Accepts a factory function that produces a Flow of byte arrays, allowing the same data source
   * to be processed multiple times if needed (useful for multi-pass compression algorithms).
   *
   * The implementation processes chunks incrementally and produces compressed output along with
   ...
   * @return [ResultStreamData] containing a Flow of compressed chunks and metadata
   *
   * Example:
   * ```
   ...
   * ```
   */
  suspend fun encode(originalSource: ByteStreamFactory): ResultStreamData<Meta>
}
