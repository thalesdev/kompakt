 ...
 *
 * Provides utilities to count byte occurrences in data streams, which is
 * essential for building optimal Huffman trees and other statistical encoders.
 */
internal object FrequencyAnalyzer {
    ...
    fun ByteArray.countFrequencies(): IntArray =
        IntArray(BYTE_RANGE).apply {
            this@countFrequencies.forEach { this[it.unsigned]++ }
        }
     ...
     */
    fun IntArray.nonZeroEntries(): List<IndexedValue<Int>> =
    ...
    fun IntArray.totalBits(codeTable: Map<Int, HuffmanCode>): Long =
        nonZeroEntries().sumOf { (byte, count) ->
            codeTable[byte]!!.length.toLong() * count
        }
}
