 ...
 *
 * Defines the contract for algorithms that can compress byte arrays
 * in-memory. Implementations should handle the entire data at once,
 * making this suitable for small to medium-sized data that fits in memory.
 *
...
interface Encoder<Meta : MetaData> {
     ...
     *
     * Processes the entire input data and produces compressed output along
     * with algorithm-specific metadata required for decompression.
     *
     * @param originalSource The uncompressed data to encode
     * @return [CompressedData] containing compressed bytes and metadata
     *
     * Example:
     * ```
     ...
     * ```
     */
    suspend fun encode(originalSource: ByteArray): CompressedData<Meta>
}
