 ...
 *
 * Returns a lambda that creates a new Flow each time it's invoked, allowing the file to be read
 * multiple times. The Flow emits byte arrays of the specified chunk size, with the last chunk
 * potentially being smaller.
 *
 * This is useful for scenarios where you need to process the same file multiple times (e.g.,
 * multi-pass compression algorithms) without loading the entire file into memory.
 *
