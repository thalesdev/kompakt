 ...
 *
 * Developed by David A. Huffman in 1952 at MIT, this algorithm builds an optimal prefix-free binary
 * code based on symbol frequencies in the input data.
 *
 * ## Algorithm Definition
 * Given:
 * - A set of symbols **S**
 * - For each symbol **x ∈ S**, a frequency **fₓ** representing the fraction of occurrences of **x**
 *   in the input data
 *
...
class HuffmanAlgorithm : CompressionCodec<HuffmanMeta>, StreamingCodec<HuffmanMeta> {
  ...
  override suspend fun encode(originalSource: ByteArray): ResultData<HuffmanMeta> {
    val frequencies = originalSource.countFrequencies()
    val tree =
        HuffmanPriorityQueue(frequencies).build()
  ...
  override suspend fun encode(originalSource: ByteStreamFactory): ResultStreamData<HuffmanMeta> {
    val frequencies = originalSource().countFrequencies()
    val tree =
        HuffmanPriorityQueue(frequencies).build()
            ?: throw IllegalStateException("No Huffman tree found")
  ...
  override suspend fun decode(compressed: ResultData<HuffmanMeta>): ByteArray {
    TODO("Not yet implemented")
  }

  override suspend fun decode(compressed: ResultStreamData<HuffmanMeta>): Flow<ByteArray> {
    TODO("Not yet implemented")
  }

  /**
   * Encodes a byte array using the provided Huffman code table.
   *
   * Writes each byte's Huffman code sequentially and returns the compressed data along with the
   * number of padding bits added to complete the final byte.
   *
   * @param uncompressed The original uncompressed data
   * @param table Map of byte values (0-255) to their Huffman codes
   * @return Pair of compressed byte array and padding bit count
   */
  ...
  /**
   * Encodes a stream of byte chunks using the provided Huffman code table.
   *
   * Processes chunks incrementally, emitting compressed data as the internal buffer fills. The
   * final chunk includes any remaining buffered data with padding bits added to complete the last
   * byte.
   *
   * @param source Factory function producing a Flow of uncompressed byte chunks
   * @param table Map of byte values (0-255) to their Huffman codes
   * @return Flow emitting compressed byte chunks
   */
  private fun encodeStreamBytes(source: ByteStreamFactory, table: Map<Int, HuffmanCode>) = flow {
