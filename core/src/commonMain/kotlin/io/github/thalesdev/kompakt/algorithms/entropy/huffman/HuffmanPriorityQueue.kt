 ...
 *
 * Maintains two sorted lists (leaves and internal nodes) to efficiently construct the Huffman tree
 * in O(n) time without requiring a heap.
 *
 * The algorithm leverages the fact that merged nodes are always added with increasing frequencies,
 * maintaining sort order in the nodes list.
 *
 * @property frequencies Array of byte frequencies (index 0-255)
 */
class HuffmanPriorityQueue(frequencies: IntArray) {
  val leaves =
      frequencies
          .withIndex()
          .filter { (_, v) -> v > 0 }
          .map { (byte, count) -> Leaf(byte = byte, count = count) }
          .sortedBy { it.count }
          .toMutableList()
   ...
   *
   * Repeatedly combines the two nodes with smallest frequencies until only the root remains.
   *
      ...
      when (size) {
        ...
        1 -> leaves.firstOrNull() ?: nodes.firstOrNull()
          ...
          while (size > 1) {
            ...
            val parent = Node(count = left.count + right.count, left = left, right = right)
